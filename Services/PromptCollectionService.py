...
class PromptCollectionService:
    ...
    def __init__(self):
        ...
        self.simulate_user_edit = (
            ...
            "visual detail. Preserve useful positive constraints, preserve useful negative constraints, "
            "and remove or rewrite constraints only when they conflict with the target facts. Prefer "
            "small, natural edits over large rewrites. If suggestions do not add reliable target "
            "information, reject them and keep the query unchanged. Produce the next search query "
            "that a careful user would submit. Use the interaction_state as memory: keep previously "
